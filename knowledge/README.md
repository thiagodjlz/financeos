...
| [documentation.md](documentation.md) | Mudancas na Central de Documentacao (menu "Sobre") — e sempre que mudar uma regra que a Central publica |
...
Valores possiveis: `auth`, `users`, `categories`, `transactions`, `dashboard`, `documentation`. Uma spec pode listar mais de um. As etapas de planejamento/implementacao da esteira automatizada (ver `specs/README.md`) usam essa lista para saber quais destes arquivos carregar — nunca a pasta inteira. `accounts`/`cards` nao sao mais dominios ativos (removidos na issue #20) — so aparecem em `domains:` numa spec que trate diretamente de reintroduzir essas funcionalidades.
