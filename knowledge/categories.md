...
- Botao "Cancelar" ao lado de "Salvar", sempre visivel (issue #28 — antes so aparecia em edicao): em criacao, limpa o formulario para o estado inicial (nome vazio, tipo Despesa, cor `#2f7d62`, icone vazio, Situacao Ativo); em edicao, tem **dois estagios** — com alteracao pendente em relacao ao snapshot capturado em `edit()`, o 1o clique restaura os valores originais e mantem a edicao; sem alteracao pendente, o clique sai da edicao e volta ao formulario de criacao. Nunca dispara requisicao HTTP. Padrao comum aos formularios de cadastro — ver `knowledge/architecture.md`.
