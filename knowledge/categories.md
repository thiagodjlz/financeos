...
- `GET /categories?type=INCOME|EXPENSE` (filtro por tipo) e consumido pelo frontend de Lancamentos para restringir o dropdown de categoria ao tipo do lancamento sendo criado/editado, e ja exclui categorias inativas (ver [transactions.md](transactions.md)) — e so filtro de exibicao no frontend, o backend de Lancamentos nao valida que a categoria escolhida bate com o tipo nem que ela esta ativa.
